...
void checkInformation(std::tm& date, std::tm& timeNow)
{
    ...
    {
        bool flag = true;
        if((date.tm_year >= timeNow.tm_year && date.tm_mon >= timeNow.tm_mon && date.tm_mday > timeNow.tm_mday)
            || (date.tm_year >= timeNow.tm_year && date.tm_mon > timeNow.tm_year)
...
void inputInformation(std::tm& date)
{
    ...
    date = timeNow;
    std::cout << std::put_time(&timeNow, "%d:%m:%Y\n");
